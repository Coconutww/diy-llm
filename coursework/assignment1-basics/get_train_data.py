...
def build_random_data_bin(
    ...
    with open(input_txt, "r", encoding="utf-8") as f:
        total_lines = sum(1 for _ in f) #按行扫描，统计整个文本文件有多少行
    print(f"总计 {total_lines} 行，正在抽取 {target_samples} 个样本...")

    # 随机生成要抽取的行号集合（排好序以便流式读取）
    # random.sample(范围，数量)，set: 把这些行号变成集合
    ...
    with open(input_txt, "r", encoding="utf-8") as f:
        ...
        for i, line in enumerate(tqdm(f, total=total_lines, desc="Processing")):
            if i in target_indices:
                line_stripped = line.strip() #strip() 会去掉字符串两端的空白字符
                if not line_stripped: #空字符，跳过处理下一行
                    continue
